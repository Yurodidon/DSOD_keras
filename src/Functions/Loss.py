...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        num_positives_per_sample = K.cast(K.sum(positive_mask, -1), tf.float32)
        ...
        negative_class_loss = tf.map_fn(
                lambda x: K.sum(tf.nn.top_k(x[0], tf.cast(x[1], dtype=tf.int32))[0]),
                elements, dtype=tf.float64)

        class_loss = tf.cast(positive_class_loss, dtype='float32') + tf.cast(negative_class_loss, dtype='float32')
        total_loss = tf.cast(class_loss, dtype=tf.float32) + tf.cast(self.alpha * positive_local_loss, dtype=tf.float32)
        ...
        num_positives_per_sample = tf.where(batch_mask, num_positives_per_sample, K.ones_like(num_positives_per_sample))
